...
<h1>Edit International Package Type</h1>
<h3>Edit </h3>
 <script  src="{{ asset('admin/ckeditor/ckeditor.js') }}" > </script>
<form action="{{ route('internationalpackage_type.update', $package_types->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
...
<script>
    function deletepackagetype(id) {
        if (confirm("Are you sure you want to delete this Package Type?")) {
            window.location = "{{ url('/Packagetype/deleteinternationalpackagetype') }}/" + id;
        }
    }
</script>
